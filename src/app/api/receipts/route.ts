...
import { db } from '@/db';
import { receipts, warehouses } from '@/db/schema';
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Date range filters
    if (dateFrom) {
      conditions.push(gte(receipts.createdAt, dateFrom));
    }
    if (dateTo) {
      // Add one day to include the entire dateTo day
      const dateToEnd = new Date(dateTo);
      dateToEnd.setDate(dateToEnd.getDate() + 1);
      conditions.push(lte(receipts.createdAt, dateToEnd.toISOString().split('T')[0]));
    }
